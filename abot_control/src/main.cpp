...
void Send2Serial(const control_msgs::FollowJointTrajectoryGoalConstPtr& a_goal, AbotServer* as, abot::Abot_control* robot)
{
    int numOfPoints = a_goal->trajectory.points.size(); /* get the number of points */
        ...
        {
            ROS_INFO("TIMES:%d",i);
            for (size_t n = 0; n < Abot->numOfJoints; n++)
            {
                Abot->abotJointstate.Js[n] = int((a_goal->trajectory.points[i].positions[n])*1000 + 0.5);
                 ROS_INFO("yaw%d:%d",n,Abot->abotJointstate.Js[n]);
                switch (n)
                {
                  case 0:
                    Abot->abotJointstate.Js[n] = 3315 - Abot->abotJointstate.Js[n];
                    break;
                  case 1:
                  case 3:
                    Abot->abotJointstate.Js[n] = 1570 - Abot->abotJointstate.Js[n];
                    break;
                  case 2:
                    Abot->abotJointstate.Js[n] = 2268 - Abot->abotJointstate.Js[n];
                    break;
                  default:
                    break;
                }
            ...
            {
                send_buf[i] = Abot->abotJointstate.Js[n]%254;
                send_buf[i+1] = Abot->abotJointstate.Js[n]/254;
                send_buf[i+2] = 0;
